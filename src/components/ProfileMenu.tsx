...
export function ProfileMenu({ theme, onTheme }: {
  ...
  return <div className="relative" ref={menuRef}>
    <button type="button" className="text-button min-w-0 gap-2 px-2" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label="Open profile menu">
      ...
      <span className="hidden max-w-28 truncate text-left xl:block"><span className="block truncate text-xs font-semibold">{user.name}</span><span className="block text-[10px] capitalize text-slate-400">{preview ? "Starter workspace" : user.role}</span></span><ChevronDown size={14} />
    </button>
